# Trabalho 1.2 — Transformações 2D, Coordenadas Homogêneas e Cores RGB

Sistema gráfico interativo 2D implementado em Python com Tkinter. O Trabalho 1.2
evolui o sistema anterior adicionando **escalonamento e rotação natural ao
redor do centro do objeto**, **rotação em torno de ponto arbitrário**, a
**composição de múltiplas transformações em uma única matriz homogênea 3×3**
aplicada de uma só vez, e uma **cor de pintura RGB** por objeto. Tudo isso
mantém os requisitos do Trabalho 1.1 (DisplayFile, Window/Viewport,
navegação, parser, etc.).
...
| Escalonamento natural (ao redor do centro do objeto) | [`core/transformations.py`](core/transformations.py:276) — `scale_around_object_center_matrix(sx, sy, obj)` + checkbox "Escala ao redor do centro do objeto" em [`main.py`](main.py:309) |
| Rotação no centro do mundo (origem) | [`core/transformations.py`](core/transformations.py:306) — `rotation_matrix(angle_degrees)` |
| Rotação no centro do objeto | [`core/transformations.py`](core/transformations.py:335) — `rotation_around_point_matrix(angle, cx, cy)` com pivô = `get_object_center(obj)` + checkbox automático "Rotação ao redor do centro do objeto" em [`main.py`](main.py:337) |
| Rotação em ponto arbitrário | [`core/transformations.py`](core/transformations.py:335) — `rotation_around_point_matrix(angle, cx, cy)` + campos `cx`/`cy` em [`main.py`](main.py:355) acionados pelo checkbox "Ponto de referência (cx, cy)" |
...
| Cor de pintura RGB por objeto | [`core/graphic_object.py`](core/graphic_object.py:12) — atributo `color` em `GraphicObject` + campo `Cor (R,G,B)` em [`main.py`](main.py:172) |
...
## 2. Matemática das Matrizes Homogêneas

### 2.1 Convenção adotada
...
### 2.3 Escala ao redor de um ponto arbitrário `(cx, cy)`
...
### 2.4 Rotação ao redor de um ponto arbitrário `(cx, cy)`
...
### 2.6 Centro geométrico do objeto
...
├── test_system.py               # Suite de testes (17 testes)
...
### 4.1 Fluxo geral de transformações
...
2. **Escolha** o tipo de transformação nos radio buttons:
   ...
   - `Rotação` — rotaciona em torno da **origem**, do **centro do
     objeto** (checkbox) ou de um pivô qualquer (via `cx`,`cy`).
3. **Preencha** os parâmetros exibidos (veja a tabela em 4.2).
...
### 4.3 Parâmetros por tipo de transformação
...
- A cor é armazenada em [`GraphicObject.color`](core/graphic_object.py:33)
  e preservada por [`apply_transformation()`](core/transformations.py:394).

### 4.5 Controles de navegação (mantidos do 1.1)

| Controle | Ação |
|---|---|
| Arrastar com botão esquerdo | Pan (arrasta a janela de visualização) |
| Roda do mouse | Zoom in/out centrado no cursor |
| Tecla `R` | Reset da visualização para o padrão |
...
### 5.1 Suite principal — `test_system.py`
...
> Os outros 10 testes cobrem `Coordinate`, `GraphicObject`, `DisplayFile`,
> `Window`, `Viewport`, `Navigator`, `Parser`, preservação de aspect ratio,
> desenho de wireframe como polígonos abertos e o pipeline completo.

### 5.2 Feature de cor — `test_color_feature.py`
...
### 5.3 Transformações ao redor do centro — `test_center_transforms.py`
...
### 5.4 Validações manuais aprovadas
...
- **Barra superior** — campo de coordenadas (`x1,y1 x2,y2 ...`), campo de
  cor (`R,G,B`), botões `Adicionar` e `Limpar Tudo`.
...
## 7. Resumo das mudanças do Trabalho 1.1 → 1.2

| Item | Trabalho 1.1 | Trabalho 1.2 |
|---|---|---|
| Escala | Sempre em torno da origem | Origem **ou** centro do objeto (checkbox) |
| Rotação | Sempre em torno da origem | Origem **ou** centro do objeto (checkbox) **ou** ponto arbitrário `(cx, cy)` |
| Cor | Não havia | Cada objeto tem `color = (r, g, b)`, com campo `Cor (R,G,B)` na interface |
| Composição | Existia `compose_matrices` | Mantida e exercitada pelos fluxos pendentes |
| Rotina genérica | `apply_transformation(obj, M)` | Mantida, agora também preserva `color` |
| Matriz | `Matrix3x3` 3×3 | Mantida |
| Testes | 17 testes | 17 + 33 asserções (cor) + 5 (centros) |
