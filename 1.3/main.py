...
Controls:
    ...
    - Window angle field + button: Rotate the navigation Window
...
class Application:
    ...
    def _setup_ui(self) -> None:
        ...
        # --- Window rotation controls ---
        ttk.Label(
            toolbar, text="Window angle (°):"
        ).pack(side="left", padx=(10, 0))
        self.window_angle_var = tk.StringVar(value="0")
        self.window_angle_entry = ttk.Entry(
            toolbar, textvariable=self.window_angle_var, width=8
        )
        self.window_angle_entry.pack(side="left", padx=(5, 0))
        self.window_angle_entry.bind("<Return>", self._on_rotate_window)

        ttk.Button(
            toolbar, text="Girar Window", command=self._on_rotate_window
        ).pack(side="left", padx=(5, 0))
        ...
        ttk.Label(
            toolbar, text=" | Window angle: girar | R: Reset | Scroll: Zoom | Drag: Pan"
        ).pack(side="left", padx=(10, 0))
    ...
    def _on_rotate_window(self, event=None) -> None:
        """Rotate the navigation Window by the angle entered in the toolbar."""
        try:
            angle = self._parse_float(
                self.window_angle_var, "ângulo da Window"
            )
        except ValueError as e:
            messagebox.showerror("Ângulo inválido", str(e))
            return

        self.window.rotate(angle)
        self._render_scene()
        self._update_status()
        self.window_angle_var.set("0")
